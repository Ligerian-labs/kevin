...
impl Projection for KohralLedgerProjection {
    ...
    async fn handle(&mut self, event: &BusEvent, conn: &mut PgConnection) -> ProjectionResult<()> {
        ...
        if terminal {
            let status = effect.status.unwrap_or(TurnStatus::Failed);
            crate::metrics::turn_terminal(status, effect.error_code.as_deref());
            tracing::info!(
                { kevin_telemetry::fields::EVENT } = kevin_telemetry::events::kohral::TURN_TERMINAL,
                run_id = %run_id,
                status = status.as_str(),
                error_code = effect.error_code.as_deref().unwrap_or_default(),
                seq,
                "Kohral turn reached a terminal status"
            );
