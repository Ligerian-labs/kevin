...
use kevin_telemetry::{events, metrics as metric_names};
...
async fn health_detailed(State(state): State<KohralState>) -> Json<Value> {
    ...
    metrics::gauge!(metric_names::KOHRAL_DRAINING).set(f64::from(u8::from(draining)));
    #[allow(clippy::cast_precision_loss)]
    let active_runs = active.unwrap_or(0) as f64;
    metrics::gauge!(metric_names::KOHRAL_TURNS_ACTIVE).set(active_runs);
    Json(json!({
...
async fn submit(state: &KohralState, headers: &HeaderMap, body: &[u8]) -> KohralResult<Response> {
    ...
    if let Some(existing) = state.ledger().by_key(&key).await? {
        if existing.request_hash != request_hash {
            metrics::counter!(metric_names::KOHRAL_TURNS_TOTAL, "outcome" => "conflict")
                .increment(1);
        ...
        metrics::counter!(metric_names::KOHRAL_TURNS_TOTAL, "outcome" => "idempotent_replay")
            .increment(1);
    ...
    metrics::counter!(metric_names::KOHRAL_TURNS_TOTAL, "outcome" => "accepted").increment(1);
