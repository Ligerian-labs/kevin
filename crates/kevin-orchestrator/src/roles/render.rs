//! A deterministic, dependency-free renderer for the markdown prompt templates
//! in `crates/kevin-orchestrator/prompts/`.
//!
//! Two constructs, both intentionally minimal so a prompt stays readable as
//! markdown and a rendered prompt is a pure function of its [`Vars`]:
//!
//! - `{{name}}` — replaced by the variable's value. An unknown name is left
//!   verbatim so a test (and a snapshot) catches the mistake instead of
//!   silently shipping a hole in a prompt.
//! - a line containing only `{{#name}}` … a line containing only `{{/name}}` —
//!   the body is kept when `name` is set to a non-blank value and dropped
//!   otherwise. The marker lines themselves never render. Sections do not
//!   nest.
//!
//! After substitution the result is trimmed and runs of blank lines are
//! collapsed to one, so dropping an optional section leaves no hole.
...
impl Vars {
    ...
    /// Sets `name` to the joined `lines` (one per line), or leaves it unset
    /// when `lines` is empty — which drops the enclosing section.
...
mod tests {
    ...
    #[test]
    fn set_lines_drops_empty_collections() {
        let mut vars = Vars::new();
        vars.set_lines("items", Vec::<String>::new());
        assert!(vars.is_blank("items"));
        vars.set_lines("items", ["- a", "- b"]);
        assert_eq!(vars.get("items"), Some("- a\n- b"));
        vars.set_opt("maybe", None::<String>);
        assert!(vars.is_blank("maybe"));
        vars.set_opt("maybe", Some("yes"));
        assert_eq!(vars.get("maybe"), Some("yes"));
    }

    #[test]
    fn tidy_collapses_blank_lines_and_trims() {
        assert_eq!(tidy("\n\na\n\n\n\nb\n\n"), "a\n\nb");
    }
}
